...
const ReservationsManagement = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [reservationsRes, customersRes] = await Promise.all([
        fetch(`${baseUrl}/admin/reservations`).then(res => res.json()),
        fetch(`${baseUrl}/admin/customers`).then(res => res.json())
      ]);
      
      setReservations(reservationsRes.reservation);
      setCustomers(customersRes.data);
      setLoading(false);
    } catch (err) {
      setError(err.message);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const baseUrl = process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000';
      const response = await fetch(`${baseUrl}/admin/reservations/add`, {
        method: 'POST',
        headers: {
  ...
  const deleteReservation = async (id) => {
    if (window.confirm('Are you sure you want to delete this reservation?')) {
      try {
        ...
        const response = await fetch(`${baseUrl}/admin/reservations/${id}`, {
          method: 'DELETE'
        });
